...
### 4.2.1 估值硬门槛（必须执行，不可跳过）

**瓶颈真实 ≠ 投资机会。一家公司可以坐在S级瓶颈上，但如果估值已经把5年增长全部定价，它就不是一个好标的。** 必须对每家公司计算以下指标并标注在报告中：

| 指标 | 红线 | 触发后果 |
|------|------|---------|
| PS（市销率） | >30x | 信号强度不得超过 ★★，必须在简评中标注"⚠️ 估值极度透支" |
| PE（市盈率） | >100x 或 亏损 | 信号强度不得超过 ★★，必须标注"⚠️ 未盈利/估值极高" |
| 近期增发后股价涨幅 | 增发后60天内股价翻倍 | 标注"⚠️ 情绪驱动特征"，信号强度降一级 |

**估值合理性检验（必做）**：对每家标的回答一个问题——"以当前市值买入，假设最乐观情景全部兑现，10年后以25x PE退出，年化回报是多少？" 如果年化回报<10%，则标注"当前价格不具备安全边际"。

**对比同行估值**：将标的的 PS/PE 与同行业已盈利公司对比。如果差距超过10倍，必须在报告中解释为什么值得这个溢价，或者标注估值风险。
...
信号强度评级（估值是硬性门槛，不可被瓶颈纯正度覆盖）：
- ★★★★★ 多重交叉验证、客户已导入、收入已体现、**PS<15x且PE<50x且盈利**
- ★★★★ 大部分验证通过、**PS<20x且PE<80x且盈利**
- ★★★ 逻辑成立但部分待验证、**估值在同行合理范围内**
- ★★ 早期信号 或 **瓶颈逻辑成立但估值已透支（PS>30x或亏损）**
- ★ 纯概念、未验证、或估值极度泡沫化
